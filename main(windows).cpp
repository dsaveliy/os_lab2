...
void runTest(int N, std::vector<int> kValues) {
    ...
    for (int k : kValues) {
        ...
        start = std::chrono::high_resolution_clock::now();
        auto wResult = multiplyMatricesByBlocksWinAPI(A, B, k);
        end = std::chrono::high_resolution_clock::now();
        double ms_winapi = std::chrono::duration<double, std::milli>(end - start).count();

        std::cout << k << '\t' << numThreads << "\t\t" << std::fixed << std::setprecision(1)
            << ms_thread << "\t\t" << ms_winapi << std::endl;
    }
}

int main() {
    runTest(20, { 20, 10, 7, 5, 4, 3, 2 });
    runTest(100, { 100, 50, 34, 25, 20, 17, 13, 12, 10 });
    runTest(200, { 200, 100, 67, 50, 40, 34, 29, 25, 23, 21 });
    return 0;
}
